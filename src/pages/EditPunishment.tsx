...
export default function EditPunishment() {
  ...
  async function save() {
    if (!id) return
    await db.punishments.update(id, { title, emoji, points })
    navigate('/config/punishments')
  }

  async function remove() {
    if (!id) return
    await db.punishments.delete(id)
    navigate('/config/punishments')
  }

  return (
    <Layout>
      <h2 className="text-lg font-bold mb-4">Редактировать наказание</h2>
